        ...
        if (entityPlayer != null && entityPlayer.getCurrentEquippedItem() != null
            ...
            if (message.keyPressed == Key.CHARGE.ordinal()) {
                ((IKeyBound) entityPlayer.getCurrentEquippedItem().getItem())
                        .doKeyBindingAction(entityPlayer, entityPlayer.getCurrentEquippedItem(), Key.CHARGE);
            } else if (message.keyPressed == Key.EXTRA.ordinal()) {
                ((IKeyBound) entityPlayer.getCurrentEquippedItem().getItem())
                        .doKeyBindingAction(entityPlayer, entityPlayer.getCurrentEquippedItem(), Key.EXTRA);
            } else if (message.keyPressed == Key.RELEASE.ordinal()) {
                ((IKeyBound) entityPlayer.getCurrentEquippedItem().getItem())
                        .doKeyBindingAction(entityPlayer, entityPlayer.getCurrentEquippedItem(), Key.RELEASE);
            } else if (message.keyPressed == Key.TOGGLE.ordinal()) {
                ((IKeyBound) entityPlayer.getCurrentEquippedItem().getItem())
                        .doKeyBindingAction(entityPlayer, entityPlayer.getCurrentEquippedItem(), Key.TOGGLE);
            }
        }

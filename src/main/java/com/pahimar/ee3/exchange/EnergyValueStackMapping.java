...
public class EnergyValueStackMapping
        implements JsonSerializer<EnergyValueStackMapping>, JsonDeserializer<EnergyValueStackMapping> {
     ...
     */
    @Override
    public EnergyValueStackMapping deserialize(JsonElement jsonElement, Type typeOfT,
        ...
        if (!jsonElement.isJsonPrimitive()) {
            ...
            if (jsonStackValueMapping.get("wrappedStack") != null) {
                try {
                    wrappedStack = new WrappedStack()
                            .deserialize(jsonStackValueMapping.get("wrappedStack").getAsJsonObject(), typeOfT, context);
            ...
            if (jsonStackValueMapping.get("energyValue") != null) {
                try {
                    energyValue = new EnergyValue()
                            .deserialize(jsonStackValueMapping.get("energyValue").getAsJsonObject(), typeOfT, context);
                } catch (JsonParseException e) {
     ...
     * JsonElements for any non-trivial field of the {@code src} object. However, you should never invoke it on the
     * {@code src} object itself since that will cause an infinite loop (Gson will call your call-back method again).
     * </p>
     *
     ...
     */
    @Override
    public JsonElement serialize(EnergyValueStackMapping energyValueStackMapping, Type typeOfSrc,

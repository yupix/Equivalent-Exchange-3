...
public class CalcinationHandler extends TemplateRecipeHandler {
    ...
    public class CachedCalcinationRecipe extends CachedRecipe {
        ...
        public CachedCalcinationRecipe(ItemStack outputDust) {
            ...
            maxEnergyValue = (outputDust.getItemDamage() < (ItemAlchemicalDust.getAlchemicalDusts().size() - 1)
                    ? EnergyValueRegistryProxy
                            .getEnergyValue(ItemAlchemicalDust.getAlchemicalDusts().get(outputDust.getItemDamage() + 1))
                    : new EnergyValue(Float.MAX_VALUE));
    ...
    public void drawExtras(int recipe) {
        ...
        if (cRecipe != null && cRecipe.minEnergyValue != null
                && cRecipe.maxEnergyValue != null
                && EnergyValueRegistryProxy.getEnergyValue(
                        ItemAlchemicalDust.getAlchemicalDusts().get(ItemAlchemicalDust.getAlchemicalDusts().size() - 1))
                        != null) {
            drawStringC(
                    StatCollector
                            .translateToLocalFormatted(
                                    "gui.nei.ee3:calcination.tooltip.2",
                                    ...
                                    (cRecipe.maxEnergyValue.getValue() <= EnergyValueRegistryProxy
                                            .getEnergyValue(
                                                    ItemAlchemicalDust.getAlchemicalDusts()
                                                            .get(ItemAlchemicalDust.getAlchemicalDusts().size() - 1))
                                            .getValue()

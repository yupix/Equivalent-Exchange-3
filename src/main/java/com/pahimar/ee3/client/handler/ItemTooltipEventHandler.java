                ...
                if (wrappedItemStack.getStackSize() > 1) {
                    event.toolTip.add(
                            String.format(
                                    ...
                                    energyValueDecimalFormat.format(energyValue.getValue()))); // TODO Localize
                    event.toolTip.add(
                            String.format(
                                    "Exchange Energy (Stack of %s): %s",
                                    event.itemStack.stackSize,
                                    energyValueDecimalFormat
                                            .format(wrappedItemStack.getStackSize() * energyValue.getValue()))); // TODO
                                                                                                                 // Localize
                } else {
                    event.toolTip.add(
                            String.format(
                        ...
                        if (EnergyValueRegistryProxy.getEnergyValueForStack(fluidStack) != null) {
                            ...
                            event.toolTip.add(
                                    String.format(
                                            ...
                                            energyValueDecimalFormat.format(fluidStackEnergyValue.getValue()))); // TODO
                                                                                                                 // Localize
                            event.toolTip.add(
                                    String.format(
                                            " - Exchange Energy (Container): %s",
                                            energyValueDecimalFormat.format(
                                                    energyValue.getValue() - fluidStackEnergyValue.getValue()))); // TODO
                                                                                                                  // Localize
                        }
                    }
                }
        ...
        if (event.itemStack.getItem() instanceof IOwnable) {
            ...
            if (playerUUID != null && UsernameCache.containsUUID(playerUUID)) {
                ...
                event.toolTip.add(
                        StatCollector.translateToLocalFormatted(
                                Messages.Tooltips.ITEM_BELONGS_TO,
                                ItemHelper.getOwnerName(event.itemStack)));
            } else {
                if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard.isKeyDown(Keyboard.KEY_RSHIFT)) {
